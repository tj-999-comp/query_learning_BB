...
「本番反映してよい」というユーザー承認が許可するのは、承認済みの同一コミットを`main`へ反映し、Productionへデプロイする操作だけである。次の操作は別承認が必要であり、暗黙に実行してはならない。

- Issueのクローズまたは完了扱い
- Pull Requestのクローズ、ブランチ削除、関連Issueの状態変更
- 作業記録の作成・公開要求（ユーザーから依頼された場合は、作業完了後に別途実施する）

本番デプロイ完了、Production URLの応答確認、CI成功、STG確認、PRマージ、受入条件の充足、作業者の判断は、Issueクローズの承認ではない。Issueをクローズする場合は、ユーザーが対象Issueを明示してクローズを依頼したことを確認する。
